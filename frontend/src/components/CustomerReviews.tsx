...
export default function CustomerReviews() {
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, i) => (
      <StarIcon
        key={i}
        filled={i < rating}
        className={`w-5 h-5 ${i < rating ? 'text-[#FFE500]' : 'text-gray-600'}`}
      />
    ))
  }
  ...
  return (
    <section className="relative py-20 sm:py-28 overflow-hidden bg-[#0072CE]">
      {/* Cosmic Background Effects */}
      <div className="absolute inset-0">
        <div className="absolute inset-0 star-field opacity-30"></div>
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#FFE500]/5 rounded-full blur-3xl"></div>
        <div className="absolute bottom-0 right-1/4 w-80 h-80 bg-[#0072CE]/5 rounded-full blur-3xl"></div>
        {/* Subtle grid pattern */}
      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          >
            <StarIcon filled className="w-4 h-4 text-[#FFE500]" />
            <span className="text-[#FFE500] text-sm font-medium tracking-wide">Customer Stories</span>
          </motion.div>
        ...
        <div className="relative max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {getVisibleReviews().map((review, index) => (
              ...
              >
                {/* Card Glow Effect */}
                <div className="absolute -inset-0.5 bg-gradient-to-r from-[#FFE500]/20 to-[#0072CE]/20 rounded-2xl blur opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                <div className="relative glass rounded-2xl p-6 border border-white/10 hover:border-[#FFE500]/30 transition-all duration-300 h-full flex flex-col">
                  {/* Quote Icon */}
                  <QuoteIcon className="absolute top-4 right-4 w-8 h-8 text-[#FFE500]/10" />

                  {/* Header */}
                  <div className="flex items-start mb-4">
                    <div className="w-12 h-12 bg-gradient-to-br from-[#FFE500] to-[#FFD700] rounded-full flex items-center justify-center text-[#0072CE] font-bold text-lg mr-4 shadow-lg shadow-[#FFE500]/20">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {review.verified && (
                          <CheckIcon className="w-4 h-4 text-[#0072CE] flex-shrink-0" />
                        )}
                      </div>
                  ...
                  {review.verified && (
                    <div className="mt-4 pt-4 border-t border-white/5">
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <CheckIcon className="w-3.5 h-3.5 text-[#0072CE]" />
          ...
          <div className="flex justify-center items-center mt-10 gap-6">
            <button
              onClick={prevReview}
              className="w-12 h-12 glass rounded-full flex items-center justify-center border border-white/10 hover:border-[#FFE500]/50 hover:bg-[#FFE500]/10 transition-all duration-300 group"
              aria-label="Previous review"
            >
              <ChevronLeftIcon className="w-5 h-5 text-gray-400 group-hover:text-[#FFE500] transition-colors" />
            </button>
              ...
              {displayReviews.map((_, index) => (
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                    index === currentReview
                      ? 'bg-[#FFE500] w-8 shadow-lg shadow-[#FFE500]/30'
            ...
            >
              <ChevronRightIcon className="w-5 h-5 text-gray-400 group-hover:text-[#FFE500] transition-colors" />
            </button>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-2">
            <CheckIcon className="w-4 h-4 text-[#0072CE]" />
